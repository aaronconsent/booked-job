...
def main():
    ...
    try:
        ...
        log(f"LinkedIn queue failed (free-plan limit or error): {ex}"); return
    done.add(nxt["id"]); state["linkedin"] = list(done)
    json.dump(state, open(STATE, "w"), indent=2)
    log(f"QUEUED '{nxt['id']}' to LinkedIn via Buffer")
    try:
        import log_change
        log_change.add("site", f"Queued to LinkedIn (Buffer): {nxt['title']}")
    except Exception:
        pass
